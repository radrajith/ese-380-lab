...
//input/output
	...
	ldi r16,0b00000111					; set up port a to 
...
line2_message8:	.db 2,"",0
line3_message8:	.db 3, "HZ        0.5SEC", 0  ; test string for line #3.
;*******************************************************************************************
...
//connect backlight control to pin a 5
//this subroutine when called will disable backlight
backlight_off:
	sbi porta, 5
	ret
//
cbi porta, 5							//to turn on the backlight
...
backlight_display:
  ...
  linechange1:
   ldi  ZH, high(line2.1_message10<<1)		;
   ldi  ZL, low(line2.1_message10<<1)		;
   rcall load_msg						; load message into buffer(s).
   rcall update_lcd_dog
;--------------------------------------
;lines to display on the lcd
...
line2.1_message10:	.db 4, "    DISABLED    ",0
...
backlight:
	...
	sbrs r12, 0
	sbrc r12, 0
	rjmp enable_backlight
	rjmp disable_backlight
...
enable_backlight:
	...
	ldi r16, $4C						;load the counter with 19507
	out OCR1AH, r16						; so that we will get 5s 
	ldi r16, $97
	out OCR1AL, r16						;
	sei									;enable global interrupts...
	...
	;TCCR1B = FOC0 : WGM11 : COM11 : COM10 : WGM11 : CS12 : CS11 : CS10 
	; 0 0 0 0 0 1 0 0 
	; FOC Off; No WF Gen; COM=Nrml Port Op; Pre-scaler= 1/256 
	ldi r16, 1<<CS12|0<<CS11|0<<CS10	; load 256 PRESCaLE TCCR0 value. 
	out TCCR1A, r16						; and start timer
