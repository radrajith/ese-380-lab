 ...
 *
 ...
 ;edges will be counted. when the pushbutton is pressed it will alternate between counting
 ;between 1/2 sec and 1 sec. To create this code, freq 2 and freq 3 subroutines are copied 
 ; from the previous 2 codes. 
...
main:
	...
	rcall frequency_meter_2				;load the timer and count the frequency
	sbrc r27, 0
	rcall frequency_meter_3
...
frequency_meter_2:
	ldi r16, $3D						;load the counter with 15625
	...
	check_edge:
		...
		mov r25,r16
		brcs check_edge			;if there is a carry then branch
...
frequency_meter_3:
	...
	out OCR1BH, r16						; so that we will get .5s 
	ldi r16, $07
	out OCR1BL, r16						;
	ldi r16, $00
	mov r8, r16
	...
	check_edge1:		
		...
		in r16, pina			;take in the current wave signal logic 
		cp r16,r25			;and compare to previous logic recorded, 
		breq check_edge1			;if it is the same then skip to tweak delay		
		mov r25, r16
		inc r8				;and then increment the counter
		brne check_edge1			;if it didnt over count then go to tweak delay
		inc r9				;if so then increment the second register
		brtc check_edge1
	finish2:
		ret
...
line3_message:	.db 3, "MGFM***HZ***1SEC", 0  ; test string for line #3.
