...
reset:
	...
	rcall adc_init
	ldi r27, 1							;initialization of r27
	set


main:
	brtc hold							;if cleared just display the existing value
	rcall conv_check					;check if the conversion is complete
	rcall unpack						;convert the answer to ascii
	rcall display						;display the measured value
	brts read_next						;if t flag is set then read the next value
	rjmp main

	hold:
		sbrs r27, 0						;skip T flag is set, meaning its in run mode
		rcall display					; wait until, the hold is released
		rjmp main	
	read_next:
...
conv_check:
	...
	//rjmp func_conv
	;
	sbis ADCSRA, ADIF					;check if the adif flag is set or conversion done
	rjmp func_conv						;when set jump to func_conv
	rjmp conv_check

	func_conv:
	//ask weather to disable the conversion and renable after flag
		ldi r16, 0<<adif
		out adcsra, r16					;reset the adif flag
		in r8, ADCL						;read the conversion values
		in r9, ADCH
...
;*****************ADC initialization**************************
 ...
 adc_init:
	...
	ldi r16, 0b11000100					; for this lab we will be using a division factor
...
switch:
	...
	and r17,r16
	com r27								;if pressed complement r27 everytime
	breq finish1
	
finish1:
	sbrs r17, 0							;check if r27 is 1
	clt									; if 0 clear t flag
	sbrc r17, 0							;check if r27 is 0
	set									;if 1 set the t flag
	pop r17
	pop r16
	out sreg, r16
	pop r16
...
line3_message:	.db 3, "RUN         VM1B", 0  ; test string for line #3.
...
line3_message0:	.db 3, "HOLD        VM1B", 0  ; test string for line #3.
...
line3_message2:	.db 3, "            VM1B", 0  ; test string for line #3.
...
mpy16u:	
	...
	ldi r18, 50				;load the multiplier, to measure the
